...
@Service
public class FwfServiceImpl implements IFwfService {
	
	@Autowired
	private SapFwfTbMapper sapFwfTbMapper;
	...
	@Override
	public int uploadFwfExcel(HttpServletRequest request, HttpServletResponse response, MultipartFile file)
			throws Exception {
		InputStream in = null;
		...
		try {
			...
			List<SapFwfTb> list=new ArrayList<SapFwfTb>();
			for (int i = 1; i < listob.size(); i++) {
				...
				SapFwfTb.setFwf(df.format(ObjectUtils.getDouble(listob.get(i).get(3))));
				SapFwfTb.setCjr(ObjectUtils.getString(listob.get(i).get(4)));
				SapFwfTb.setLrzx(ObjectUtils.getString(listob.get(i).get(5)));
				SapFwfTb.setSklx(ObjectUtils.getString(listob.get(i).get(6)));
				list.add(SapFwfTb);
			}
			
			List<SapFwfTb> lists=new ArrayList<SapFwfTb>();
			for (int i = 0; i < list.size(); i++) {
		        ...
		        if(ObjectUtils.getString(dj1.get(0).get("_widget_1545377155914")).equals("PD")){
		        	SapFwfTb SapFwfTb=sendSAPPost(list.get(i),ObjectUtils.getString(dj1.get(0).get("_widget_1586139868538")));
		        	if(!"".equals(SapFwfTb.getPzbh()) && SapFwfTb.getPzbh()!=null){//判断同步到SAP是否成功
		        		Map<String, Object> resmap=getSapFwfTb(SapFwfTb,dj1,df1_api);
		        		if(!"".equals(ObjectUtils.getString(resmap.get("_widget_1548037673456")))){//判断是否同步到DF1
		        			SapFwfTb.setJdybm(ObjectUtils.getString(resmap.get("_widget_1548037673456")));
			        		SapFwfTb.setJxsmc(ObjectUtils.getString(dj1.get(0).get("_widget_1543814802008")));
		        			addZHKDM1(resmap);
		        			pushZHKDDJQR(resmap);
		        		}
			        	lists.add(SapFwfTb);
			...
			num=sapFwfTbMapper.insertByBatch(lists);
...
//		SapFwfTbExample example = new SapFwfTbExample();
//		Criteria criteria=example.createCriteria();
//		criteria.andIdIn(list);
//		List<SapFwfTb> sapDskqzTbs=SapFwfTbMapper.selectByExample(example);
//		for (int i = 0; i < sapDskqzTbs.size(); i++) {
//			SapFwfTbMapper.updateByPrimaryKey(sendSAPPost01(sapDskqzTbs.get(i)));
//		}
//		
//	}
	...
	public EUDataGridResult getSAPPost(int page, int rows,String status,String zkrq) {
		...
		List<SapFwfTb> item = sapFwfTbMapper.selectByExample(example);
		// 创建一个返回值对象
		EUDataGridResult result = new EUDataGridResult();
		result.setRows(item);
		// 取记录总条数
		PageInfo<SapFwfTb> pageInfo = new PageInfo<>(item);
	...
	public int delete(List<Integer> list) {
		...
		Criteria criteria=example.createCriteria();
		criteria.andIdIn(list);
		int num=sapFwfTbMapper.deleteByExample(example);
		return num;
	}
	
	
	//同步到SAP过账（含清账）补4.9%服务费给经销商的收款单生成凭证
	public SapFwfTb sendSAPPost(SapFwfTb SapFwfTb,String jxsbm){
		try {
			...
			token6.setValue(SapFwfTb.getDjbh());
			...
			documentHeaderText.setDocumentHeaderText(SapFwfTb.getSkbz());
			...
			token7.setValue(SapFwfTb.getCjr());
			...
			//服务费
			DecimalFormat df = new DecimalFormat("#0.00");
		    String str=df.format(Double.parseDouble(SapFwfTb.getFwf().trim())/1.06);
			...
			itemParam.setDocumentItemText(SapFwfTb.getSkbz());
			...
			token23.setValue(SapFwfTb.getLrzx());
			...
			amountItem1.setAmountContent(new BigDecimal(Double.parseDouble(SapFwfTb.getFwf().trim())-Double.parseDouble(str))); 
			...
			itemParam1.setDocumentItemText(SapFwfTb.getSkbz());
			...
			amountCreditorItem2.setAmountContent(new BigDecimal(-Double.parseDouble(SapFwfTb.getFwf().trim())));
			...
			itemParam2.setDocumentItemText(SapFwfTb.getSkbz());
			...
			if("0000000000".equals(documentNo)){
				...
				SapFwfTb.setPzbh("");
				SapFwfTb.setStatus("n");
				SapFwfTb.setMsg(set.toString());
			}else{
				SapFwfTb.setPzbh(documentNo);
				SapFwfTb.setStatus("y");
				SapFwfTb.setMsg("");
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public Map<String, Object> getSapFwfTb(SapFwfTb SapFwfTb,List<Map<String, Object>> dj1,JDYAPIUtils df1_api) throws ParseException{
		Map<String, Object> rawData = new HashMap<String, Object>();
		Map<String, Object> m1 = new HashMap<String, Object>();
		m1.put("value",SapFwfTb.getDjbh());
		rawData.put("_widget_1611404421677", m1);// 导入单据编号
		Map<String, Object> m2 = new HashMap<String, Object>();
		m2.put("value",SapFwfTb.getPzbh());
		rawData.put("_widget_1593141119943", m2);// 凭证编号
		Map<String, Object> m4 = new HashMap<String, Object>();
		m4.put("value",SapFwfTb.getCjr());
		...
		m30.put("value",SapFwfTb.getSkbz());
		...
		m33.put("value",SapFwfTb.getSklx());
    ...
    private void pushZHKDDJQR(Map<String, Object> resmap){
    	try {
	        ...
	        String ydcl = "0.00%";
	        if(m1.size()!=0){
	        	mb=ObjectUtils.getDouble(m1.get(0).get("_widget_1565166083774"));//目标
	        	DecimalFormat df = new DecimalFormat("#0.00");
...
//			actioncard.setBtnOrientation("0");
	...
	@Override
	public XSSFWorkbook getXSSFWorkbook(String excelUrl,String fwfStatus, String zkrq) {
		XSSFWorkbook wb = null;
		try {
			SapFwfTbExample example = new SapFwfTbExample();
			Criteria criteria=example.createCriteria();
			if(fwfStatus!=null && !"".equals(fwfStatus)){
				criteria.andStatusEqualTo(fwfStatus);
			}
			if(zkrq!=null && !"".equals(zkrq)){
				criteria.andZkrqLike(zkrq+"%");
			}
			...
			for (SapFwfTb SapFwfTb : item) {
				...
				cell0.setCellValue(SapFwfTb.getJxsbm());

				XSSFCell cell1 = row.getCell(1);
				if (null == cell1) {
					cell1 = row.createCell(1);
				}
				cell1.setCellValue(SapFwfTb.getZkrq());

				XSSFCell cell2 = row.getCell(2);
				if (null == cell2) {
					cell2 = row.createCell(2);
				}
				cell2.setCellValue(SapFwfTb.getSkbz());

				XSSFCell cell3 = row.getCell(3);
				if (null == cell3) {
					cell3 = row.createCell(3);
				}
				cell3.setCellValue(SapFwfTb.getFwf());
				
				XSSFCell cell4 = row.getCell(4);
				if (null == cell4) {
					cell4 = row.createCell(4);
				}
				cell4.setCellValue(SapFwfTb.getCjr());
				
				XSSFCell cell5 = row.getCell(5);
				if (null == cell5) {
					cell5 = row.createCell(5);
				}
				cell5.setCellValue(SapFwfTb.getLrzx());
				
				XSSFCell cell6 = row.getCell(6);
				if (null == cell6) {
					cell6 = row.createCell(6);
				}
				cell6.setCellValue(SapFwfTb.getSklx());
