 ...
 */
@Controller
public class MesQrcodeController {

	@Autowired
	private ISapScpgService sapScpgService;
	
	// OA华生集团 -- 生产派工单
	static String appId = "5c048a8379332d0978a68b8e";
	static String entryId = "5f3669a5b97b0a00069aa49d";
	static String apiKey = "SGmAqjtntz3q5M0rZqm2b4nGlRpSlah6";

	// OA华生集团 -- 物料组交货周期
	static String wlz_entryId = "5f5b4063dfea1f0006dad122";
	// OA华生集团 -- 床头柜产品配置
	static String ctg_entryId = "5fa245999fbdc30006c675ec";//床头柜产品配置名单
	
	// OA华生集团 -- 产品名称打印对照表
	static String dydz_entryId = "60a6286dccaf180007b9ad43";
	
//	@RequestMapping("/uploadMesQrcodeExcel")
//	@ResponseBody
//	public MessageResult uploadWlExcel(@RequestParam("mesQrcodeExcelFile") MultipartFile mesQrcodeExcelFile,HttpServletRequest request, HttpServletResponse response){
...
//				wlzmap.put(getStr(lists.get(i).get("_widget_1599815780035")), Long.valueOf(getStr(lists.get(i).get("_widget_1599815780069"))));
//			}
//	        
//			if (mesQrcodeExcelFile==null) {
//				return MessageResult.build(null,"上传Excel失败,请重试！");
//			}
//			InputStream in = null;
//			List<List<Object>> listob = null;
//			
//			in = mesQrcodeExcelFile.getInputStream();
//			listob = new ExcelUtil().getBankListByExcel(in, mesQrcodeExcelFile.getOriginalFilename());
//			//从Excel中取出销售订单，并去重
//	        Set<String> sets=new HashSet<String>();//存放销售订单
//	        for (int i = 0; i < listob.size(); i++) {
//	        	sets.add(listob.get(i).get(4).toString());
//			}
//	        
//	        
//	        for (String SalesOrder:sets) {
//	        	//循环查询每一个销售订单
//	        	JSONArray array=getJSONArray("https://my300201-api.saps4hanacloud.cn/sap/opu/odata/sap/YY1_SALESDOCUMENTTRACKAPI_CDS/YY1_SalesDocumentTrackAPI?$filter=SalesDocument%20eq%20'"+SalesOrder+"'");
...
//	        			return MessageResult.build(500,"简道云J3物料组交货周期表中没有物料组编码"+wlzmap.get(materialGroup)+"！");
//	        		}
...
//			try {
//				for (int i = 0; i < listob.size(); i++) {
//					int num=Integer.valueOf(getStr(listob.get(i).get(15)).substring(0, getStr(listob.get(i).get(15)).indexOf(".")));
//					ddTotal+=num;
//				}
//				for (int i = 0; i < listob.size(); i++) {
//					int num=Integer.valueOf(getStr(listob.get(i).get(15)).substring(0, getStr(listob.get(i).get(15)).indexOf(".")));
...
//						}
//						
//						String qrCode=getStr(listob.get(i).get(4))+getStr(listob.get(i).get(5))+getStr(listob.get(i).get(0))+str;
...
//				        if(list.size()==0){
//				        	Map<String, Object> rawData=getExcelData(listob.get(i),qrCode,str,wlzmap,barcode,jhzqs,ctg_list);
//							api.createForData(rawData);
//							total++;
//				        }else if(list.size()==1){
//				        	Map<String, Object> rawData=getExcelUpdateData(listob.get(i),qrCode,str,wlzmap,jhzqs);
...
//	}
	...
	@RequestMapping("/addMesQrcodeTb")
	@ResponseBody
	public MessageResult addMesQrcodeTb(){
		Map<String,Long> jhzqs=new HashMap<String,Long>();
		JDYAPIUtils wlz_api = new JDYAPIUtils(appId, wlz_entryId, apiKey);
		JDYAPIUtils ctg_api = new JDYAPIUtils(appId, ctg_entryId, apiKey);
		...
		try {
			//找出物料组中所有的交货周期
	        List<Map<String, Object>> lists= wlz_api.getAllFormData(null,null);
	        for (int i = 0; i < lists.size(); i++) {
	        ...
	        for (String SalesOrder:sets) {
	        	//循环查询每一个销售订单
	        	JSONArray array1=getJSONArray("https://my300201-api.saps4hanacloud.cn/sap/opu/odata/sap/YY1_SALESDOCUMENTTRACKAPI_CDS/YY1_SalesDocumentTrackAPI?$filter=SalesDocument%20eq%20'"+SalesOrder+"'");
	        	Set<String> sets1=new HashSet<String>();//获取销售订单中每个物料的物料组编码
	        	for (int i = 0; i < array1.size(); i++) {
	        		sets1.add(array1.getJSONObject(i).getString("MaterialGroup"));
				}
	        	//用物料组编码找出对应的交货周期
	        	List<Long> lists2=new ArrayList<Long>();
	        	for (String materialGroup : sets1) {
	        		if(wlzmap.get(materialGroup)==null){
	        			return MessageResult.build(500,"简道云J3物料组交货周期表中没有物料组编码"+materialGroup+"！");
	        		}
	        		lists2.add(wlzmap.get(materialGroup));
				}
	        	if(lists2==null || lists2.size()==0){
	...
	public Map<String, Object> getUpdateData(JSONObject json,String qrCode,String str,Map<String,Long> map,Map<String,Long> jhzqs,Map<String,String> dydzmap){
		...
		try {
			...
			if(list.size()==1){
				Map<String, Object> m21 = new HashMap<String, Object>();
				m21.put("value", "");
				rawData.put("_widget_1597219747013", m21);// 物料名称-打印产品名称
				Map<String, Object> m22 = new HashMap<String, Object>();
				m22.put("value", "");
				rawData.put("_widget_1597219747196", m22);// 物料名称-皮号/色号
				Map<String, Object> m23 = new HashMap<String, Object>();
				...
				Map<String, Object> m39 = new HashMap<String, Object>();
				m39.put("value", "");
				rawData.put("_widget_1621825010924", m39);// 物料名称-产品名称
				Map<String, Object> m41 = new HashMap<String, Object>();
				m41.put("value", "");
				rawData.put("_widget_1621825011671", m41);// 床架全真皮/半真皮/无真皮标识
			}else if(list.size()==2){
				...
				Map<String, Object> m39 = new HashMap<String, Object>();
				m39.put("value", getBgjDyCpmc(json.getString("ProductGroup"),list.get(1),list.get(0),dydzmap));
				rawData.put("_widget_1621825010924", m39);// 物料名称-打印产品名称
				Map<String, Object> m41 = new HashMap<String, Object>();
				m41.put("value", "");
				rawData.put("_widget_1621825011671", m41);// 床架全真皮/半真皮/无真皮标识
			}else if(list.size()==3){
...
//	public Map<String, Object> getExcelData(List<Object> listExcel,String qrCode,String str,Map<String,Long> map,String barcode,Map<String,Long> jhzqs,List<Map<String, Object>> ctg_list){
//		Map<String, Object> rawData=null;
//		try {
//			List<String> list=Arrays.asList(getStr(listExcel.get(11)).split("/"));
//			rawData = new HashMap<String, Object>();
//			Map<String, Object> m1 = new HashMap<String, Object>();
//			m1.put("value", qrCode);
//			rawData.put("_widget_1597219749750", m1);// 二维码数据
//			Map<String, Object> m3 = new HashMap<String, Object>();
//			m3.put("value", listExcel.get(0));
//			rawData.put("_widget_1597218715227", m3);// 订单
//			Map<String, Object> m6 = new HashMap<String, Object>();
//			m6.put("value", str);
//			rawData.put("_widget_1597218715274", m6);// 流水号
...
//			rawData.put("_widget_1597218715486", m7);// 品牌
//			Map<String, Object> m8 = new HashMap<String, Object>();
//			m8.put("value", listExcel.get(2));
//			rawData.put("_widget_1597219257856", m8);// 计划下达日期
//			Map<String, Object> m9 = new HashMap<String, Object>();
//			m9.put("value", listExcel.get(3));
//			rawData.put("_widget_1597219257966", m9);// 实际下达日期
//			Map<String, Object> m10 = new HashMap<String, Object>();
//			m10.put("value", DateUtils.getLongToString(json.getString("MfgOrderItemPlndDeliveryDate"),DateUtils.FORMAT_1_STRING));
//			rawData.put("_widget_1597395915492", m10);// 计划交货日期
...
//				Map<String, Object> m21 = new HashMap<String, Object>();
//				m21.put("value", "");
//				rawData.put("_widget_1597219747013", m21);// 物料名称-产品名称
//				Map<String, Object> m22 = new HashMap<String, Object>();
//				m22.put("value", "");
//				rawData.put("_widget_1597219747196", m22);// 物料名称-皮号/色号
//				Map<String, Object> m23 = new HashMap<String, Object>();
...
//				rawData.put("_widget_1597219747013", m21);// 物料名称-产品名称
//				Map<String, Object> m22 = new HashMap<String, Object>();
//				m22.put("value", json.getString("ProductDescription").substring(json.getString("ProductDescription").indexOf(list.get(1))+list.get(1).length()+1, json.getString("ProductDescription").lastIndexOf("/")));
//				rawData.put("_widget_1597219747196", m22);// 物料名称-皮号/色号
//				
//				Map<String, Object> m23 = new HashMap<String, Object>();
//				m23.put("value", json.getString("ProductDescription").substring(json.getString("ProductDescription").lastIndexOf("/")+1, json.getString("ProductDescription").length()));
//				rawData.put("_widget_1597219747075", m23);// 物料名称-产品规格
//			}
//			
//			
...
//	}
	...
	public Map<String, Object> getExcelUpdateData(JSONObject json,String qrCode,String str,Map<String,Long> map,Map<String,Long> jhzqs){
		Map<String, Object> rawData=null;
		try {
			List<String> list=Arrays.asList(json.getString("ProductDescription").split("/"));
			rawData = new HashMap<String, Object>();
